"""
	Python script to train a 2D CNN to perform DOA estimation from the spectrograms of each microphone signal
	and analyze its performance.

	File name: 1sourceTracking_fromSpectrograms.py
...
model_name = 'cnn2D' # Only for the output filenames, change it also in Network declaration cell
extra_notes = 'fromSpect'
...
# %% Network declaration
net = at_models.Cnn2D(K//2, 2*N)  # Only the positive frequencies and 2 channels per microphone (magnitude and phase)
...
for j in range(len(SNR)):
	...
	legend += ['From spectrograms (SNR={})'.format(SNR[-j-1])]
...
if array_locata_name != '' and array_locata_name is not None:
	...
	print('From spectrograms rmsae: ' + str(rmsae))
