"""
	Python script to train a 1D CNN to perform DOA estimation from the coordinates of the SRP-PHAT maximums
	and analyze its performance.

	File name: 1sourceTracking_fromMaximums.py
...
# %% Network declaration
net = at_models.Cnn1D(2)  # A time sequence for the elevation coordinates and another for the azimuth
learner = at_learners.OneSourceTrackingFromMaximumsLearner(net, N, K, res_the, res_phi, array_setup.mic_pos, fs,
														   arrayType=array_setup.arrayType, apply_vad=True)
learner.cuda()
...
for i in range(len(T60)):
	for j in range(len(SNR)):
		...
		acoustic_scenes[i,j], rmsae[i,j] = learner.predict_dataset(dataset_test, trajectories_per_batch, return_rmsae=True, save_x=True, x_filed_name='max_coors')
...
for j in range(len(SNR)):
	...
	legend += ['From maximums (SNR={})'.format(SNR[-j-1])]
...
if array_locata_name != '' and array_locata_name is not None:
	...
	acoustic_scenes_locata = learner.predict_dataset(dataset_locata, 1, save_x=True, x_filed_name='max_coors')
	...
	print('From maximums rmsae: ' + str(rmsae))
